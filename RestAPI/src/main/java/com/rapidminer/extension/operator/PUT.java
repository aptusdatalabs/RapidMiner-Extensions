...
public class PUT extends Operator {
	...
	public void doWork() throws OperatorException, UndefinedParameterError {
		...
		case 0:
			...
			try {
				jsonResponse = Unirest.put(url).headers(head).body(text1).asString();
		...
		case 1:
			...
			case 0:
				...
				try {
					jsonResponse = Unirest.put(url).headers(head).body(text).asString();
				} catch (UnirestException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
	...
	public List<ParameterType> getParameterTypes() {
		...
		types.add(new ParameterTypeCategory("body category", "Body for PUT method", bodyarray, 0));
